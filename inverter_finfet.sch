...
N 210 90 310 90 {
lab=nfet_out}
C {asap_7nm_pfet.sym} 190 40 0 0 {name=pfet1 model=asap_7nm_pfet spiceprefix=X l=7n nfin=\{P_nfin\}}
C {asap_7nm_nfet.sym} 190 140 0 0 {name=nfet1 model=asap_7nm_nfet spiceprefix=X l=7n nfin=\{N_nfin\}}
...
C {vsource.sym} -80 100 0 0 {name=V1 value="pulse(\{Vlo\} \{Vhi\} 20p 10p 10p 20p 500p 1)" savecurrent=false}
C {gnd.sym} -80 130 0 0 {name=l2 lab=GND}
C {lab_pin.sym} -80 70 0 0 {name=p2 sig_type=std_logic lab=nfet_in}
C {vsource.sym} 50 200 0 0 {name=V2 value=0.7 savecurrent=false}
C {gnd.sym} 50 230 0 0 {name=l3 lab=GND}
C {lab_pin.sym} 50 170 0 0 {name=p3 sig_type=std_logic lab=vdd}
C {code_shown.sym} 430 20 0 0 {name=s1 only_toplevel=false value="

.param VDD = 0.7
* g3grau = 103+51+103+114+97+117=585  -> 58.5mV
.param Vuniq = 58.5m
.param Vlo = '0 + Vuniq'
.param Vhi = 'VDD + Vuniq'
.param N_nfin = 14
...
.control
    *.dc V1 0 0.7 1m
    ...
    alterparam N_nfin=20
    alterparam P_nfin=20
    reset
    ...
    * Transconductance  .. handle with care .. do these definitions make any sense?
    ...
    meas dc gm_max max gm from = 1m to = 690m
    ...
    * Propagation delay (on VDD/2)
    meas tran tpir when nfet_in = 'VDD/2' rise = 1
    meas tran tpof when nfet_out = 'VDD/2' fall = 1
    let tplh = tpof - tpir
    meas tran tpor when nfet_in = 'VDD/2' fall = 1
    meas tran tpif when nfet_out = 'VDD/2' rise = 1
    let tphl = tpof - tpir
    let tp = (tplh + tphl) / 2
    print tplh tphl tp 
    * Rise and fall times
    ...
    let energy = id_charge * 0.7    ; VDD seems to be an array here??
    let power = energy
